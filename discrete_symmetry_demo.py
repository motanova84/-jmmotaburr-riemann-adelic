...
import numpy as np
from sympy import N, Symbol, diff, lambdify, log, pi, simplify, sin, solve, symbols
...
def main():
    ...
    E = alpha / R**4 + beta * zeta_prime / R**2 + gamma * Lambda**2 * R**2 + delta * sin(log(R) / log(pi)) ** 2
    ...
    print("-" * 70)
    params = {alpha: 1.0, beta: -0.5, gamma: 0.01, delta: 0.1, zeta_prime: -1.46035450880958681, Lambda: 1.0}  # ζ'(1/2)
    ...
    for n in range(-2, 4):
        ...
        if 0.3 <= R_boundary <= 50:
            ...
            ax1.text(R_boundary, ax1.get_ylim()[1] * 0.95, f"$\\pi^{{{n}}}$", ha="center", fontsize=10, color="gray")
    ...
    ax1.set_title("Vacuum Energy with Discrete Symmetry: Complete Landscape", fontsize=16, pad=20)
    ...
    ax2.plot(R_range, A_range, "r-", linewidth=2, label="$A(R_\\Psi) = \\sin^2(\\log R_\\Psi / \\log \\pi)$")
    ...
    ax2.set_title("Oscillatory Amplitude from Discrete Symmetry (Fundamental Mode m=1)", fontsize=14, pad=15)
