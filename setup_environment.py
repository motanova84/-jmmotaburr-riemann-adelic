    ...
    try:
        ...
        if os.path.exists("requirements.txt"):
            result = subprocess.run(
                [sys.executable, "-m", "pip", "install", "-r", "requirements.txt"], capture_output=True, text=True
            )
...
def run_basic_tests() -> bool:
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "-m", "pytest", "tests/", "-v", "--tb=short"], capture_output=True, text=True, timeout=120
        )
...
def main():
    ...
    parser.add_argument("--full-setup", action="store_true", help="Perform full setup including data download")
