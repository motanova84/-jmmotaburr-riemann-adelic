...
def main():
    ...
    test_success = run_command("python -m pytest tests/ -v --tb=short -x", "Running test suite", timeout=120)
    ...
    fetch_success = run_command("python utils/fetch_odlyzko.py --validate-only", "Data validation", timeout=30)
    ...
    setup_success = run_command(
        "python setup_environment.py --validate-only", "Environment setup validation", timeout=60
    )
