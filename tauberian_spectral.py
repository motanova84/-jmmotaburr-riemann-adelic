...
def tauberian_spectral_computation(N, h, num_jobs=8):
    ...
    def compute_kernel_element(t, s):
        ...
        try:
            tail_bound = mp.quad(lambda u: mp.exp(u - h * u**2 / 4 - u / 2), [mp.log(P), mp.inf])
    ...
    results = Parallel(n_jobs=num_jobs, verbose=10)(
        delayed(compute_matrix_element)(i, j) for i in range(N) for j in range(i, N)
    )
...
def validatio_tauberiana():
    ...
    for i, (z, target) in enumerate(zip(zeros, targets)):
        ...
        bound = float(
            mp.exp(-h / 4)
            / (2 * target * mp.sqrt(4 * mp.pi * h))
            * mp.exp(-mp.pi / 2 * mp.sqrt(N / mp.log(N)) * (1 + zeta(3) / (2 * mp.pi * mp.log(N))))
        )

        status = "✅" if error < bound else "⚠️"
        print(
            f"Zero {i+1:2d}: {γ_computed:.12f} | Target: {target:.12f} | Error: {error:.2e} | Bound: {bound:.2e} {status}"
        )
