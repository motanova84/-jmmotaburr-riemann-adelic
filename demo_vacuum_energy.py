...
def demonstrate_equation():
    ...
    calc = VacuumEnergyCalculator(alpha=1.0, beta=1.0, gamma=0.001, delta=0.5, Lambda=1.0, precision=50)
    ...
    R_opt, f0 = derive_f0_noncircular(alpha=1.0, beta=1.0, gamma=0.001, delta=0.5, Lambda=1.0, target_f0=141.7001)
...
def visualize_energy():
    ...
    for n in range(-1, 4):
        ...
        if R_pi >= R_values[0] and R_pi <= R_values[-1]:
            ...
            ax1.text(R_pi, ax1.get_ylim()[1] * 0.95, f"π^{n}", ha="center", fontsize=10, color="red")
    ...
    for n in range(0, 3):
        ...
        if R_pi >= R_detail[0] and R_pi <= R_detail[-1]:
            ...
            ax3.text(R_pi, ax3.get_ylim()[1] * 0.9, f"π^{n}", ha="center", fontsize=10, color="red")
    ...
    plt.suptitle(
        "Ecuación del Vacío Cuántico - Acto II: Corrección Adélica Fractal", fontsize=16, fontweight="bold", y=0.995
    )
