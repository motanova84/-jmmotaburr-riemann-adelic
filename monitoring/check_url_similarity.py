...
def check_url_for_fingerprints(url: str, fingerprints: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        response = requests.get(url, timeout=30, headers={"User-Agent": "Mozilla/5.0 (Research Monitoring Bot)"})
        ...
        for snippet_name, snippet_info in fingerprints.get("latex_snippets", {}).items():
            ...
            if snippet_content in content:
                result["snippet_matches"].append({"name": snippet_name, "content": snippet_content})
...
def main():
    ...
    with open(output_file, "w") as f:
        json.dump({"timestamp": timestamp, "checked_urls": len(results), "results": results}, f, indent=2)
