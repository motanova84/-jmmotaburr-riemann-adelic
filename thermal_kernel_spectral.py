...
def build_H_operator(n_basis=20, t=0.001):
    ...
    basis_info = {"gamma_estimates": gamma_estimates, "lambda_diagonal": lambda_diagonal, "t": t, "n_basis": N}
...
def validate_spectral_construction(n_basis=20, t=0.01, max_zeros=10, verbose=True):
    """
    ...
    for i in range(n_compare):
        ...
        if verbose:
            print(f"{i+1:<8} {gamma_comp:<15.6f} {gamma_true:<15.6f} " f"{error:<15.6f} {rel_error:<12.6e}")
...
def convergence_study(n_basis_values=[10, 15, 20, 25], t_values=[0.1, 0.05, 0.01, 0.005], max_zeros=5):
    """
    ...
    results = {"n_basis": [], "t": [], "mean_error": [], "mean_rel_error": []}

    for n_basis in n_basis_values:
        for t in t_values:
            print(f"Testing n_basis={n_basis}, t={t}...")
            try:
                result = validate_spectral_construction(n_basis=n_basis, t=t, max_zeros=max_zeros, verbose=False)
                ...
                print(f"  → Mean error: {result['mean_error']:.6e}, " f"Rel error: {result['mean_rel_error']:.6e}")
...
def plot_results(result, filename="thermal_kernel_validation.png"):
    ...
    ax.plot(indices, result["true_gammas"][:n_compare], "o-", label="Odlyzko zeros", markersize=6)
    ax.plot(indices, result["computed_gammas"][:n_compare], "x-", label="Computed zeros", markersize=6)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Thermal Kernel Spectral Operator for RH Validation")
    parser.add_argument("--n_basis", type=int, default=20, help="Number of basis functions (matrix size)")
    parser.add_argument("--t", type=float, default=0.01, help="Thermal parameter (smaller = better)")
    parser.add_argument("--max_zeros", type=int, default=10, help="Number of zeros to compare")
    ...
    if args.convergence:
        ...
        convergence_study(n_basis_values=[10, 15, 20, 25], t_values=[0.1, 0.05, 0.01, 0.005], max_zeros=args.max_zeros)
    else:
        # Single validation run
        result = validate_spectral_construction(n_basis=args.n_basis, t=args.t, max_zeros=args.max_zeros, verbose=True)
