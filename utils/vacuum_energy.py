...
class VacuumEnergyCalculator:
    ...
    def find_minima(self, R_range: Tuple[float, float] = (0.1, 100.0), num_points: int = 1000) -> np.ndarray:
        """
        ...
        """
        R_values = np.logspace(np.log10(R_range[0]), np.log10(R_range[1]), num_points)
    ...
    def fundamental_frequency(
        self, R_psi: float, c: float = 299792458.0, normalization: Optional[float] = None  # Speed of light in m/s
    ) -> float:
        """
