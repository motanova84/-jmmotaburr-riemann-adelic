...
def demo_ml_pattern_recognition():
    ...
    local_density = np.array([np.sum((t_values > t - 10) & (t_values < t + 10)) for t in t_values])
...
def demo_numexpr_performance():
    ...
    pi = np.pi
    start = time.time()
    result_numexpr = ne.evaluate(
        "exp(-(x**2 + y**2) / (2*z**2)) / (sqrt(2*pi) * z)", local_dict={"x": x, "y": y, "z": z, "pi": pi}
    )
